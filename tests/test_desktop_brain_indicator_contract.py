...
def test_desktop_brain_indicator_uses_core_readiness_snapshot() -> None:
    ...
    runtime_behaviors = (root / "desktop" / "BabyAI.Desktop" / "MainWindow.RuntimeBehaviors.cs").read_text(encoding="utf-8")
    ...
    assert 'local:BrainStatusBehavior.Enabled="True"' not in xaml
    assert "BrainStatusBehavior.SetEnabled(BrainText, true)" in runtime_behaviors
